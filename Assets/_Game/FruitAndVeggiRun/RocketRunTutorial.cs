...
///   3. wrong fruit spawns, 2s later -> "wrong fruit costs a life"
///   4. water approaches,  2s later  -> "water costs a life"
...
public class RocketRunTutorial : MonoBehaviour
{
    ...
    [Tooltip("Wrong-answer mark, used as a fallback on the 'wrong fruit costs a life' step.")]
    ...
    [Tooltip("Seconds the wrong fruit is visible before the warning interrupts.")]
    [SerializeField] private float wrongFruitLeadTime = 2f;
    ...
    [Tooltip("Seconds of flight the wrong fruit should still be ahead by when the warning appears.")]
    [SerializeField] private float wrongFruitSpareTime = 1.5f;
    ...
    private IEnumerator RunTutorial()
    {
        ...
        // ---- 3. wrong fruit costs a life ----
        string wrongName = PickWrongName(targetName);
        Sprite wrongFruitIcon = (level != null && !string.IsNullOrEmpty(wrongName))
            ? level.GetProduceSprite(wrongName)
            : null;

        // spawn it far enough out that it's still ahead of the rocket when the warning fires
        SpawnAhead(wrongName, false, wrongFruitLeadTime + wrongFruitSpareTime);
        yield return new WaitForSeconds(wrongFruitLeadTime);
...
/// <summary>True once the rocket has clearly passed this pickup without grabbing it.</summary>
    ...
    /// <summary>Any produce name that isn't the one the player is currently hunting.</summary>
    private string PickWrongName(string targetName)
    {
        for (int i = 0; i < GameDefs.Names.Length; i++)
        {
            if (GameDefs.Names[i] != targetName) return GameDefs.Names[i];
        }
        return null;
    }
